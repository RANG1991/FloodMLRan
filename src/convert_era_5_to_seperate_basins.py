...
def parse_single_basin_precipitation(station_id, basin_data, discharge_file_name,
    ...
    for year in range(year_start, year_start + 1):
        ...
        datetimes = [(starting_date + datetime.timedelta(hours=int(ti[i]))) for i in range(0, len(ti))]
    ...
    for i in range(percip_mean_lat_lon.shape[0]):
        if datetimes[i].hour != 1 and i > 0:
            percip_mean_lat_lon_new.append(percip_mean_lat_lon[i] - percip_mean_lat_lon[i - 1])
        else:
